...
- **Docker + Postgres** (real deployment target) — instructions below in this file
- **Desktop app, no Docker** (single-user demo/testing on Windows or Mac) — see [`DESKTOP_BUILD.md`](./DESKTOP_BUILD.md)
...
**Done (Desktop build — Windows/Mac, no Docker, single-user demo):**
- See `DESKTOP_BUILD.md` for the full picture — this is a separate,
  additive build path, not a replacement for the Docker/Postgres setup
- SQLite instead of Postgres: added a portable `GUID`/`PortableJSON`
  column type (`backend/app/models/types.py`) that all 31 models now use,
  so the exact same model code runs against either database
- Auto-bootstrap on first launch: creates tables directly from models
  (no Alembic involved in this path) and a default admin account
- PyInstaller bundles the backend; Electron wraps it with the built
  frontend served from the same process/port
- ⚠️ **Not run even once** — no Windows/Mac/Node/Python/PyInstaller
  available while building this. Treat the first build as a debugging
  session; `DESKTOP_BUILD.md` lists the most likely failure points.
- Also fixed while doing this: `audit_log.py` was missing a `DateTime`
  import (pre-existing bug, would have crashed on load), and
  `database.py` passed Postgres-only pool kwargs unconditionally (would
  have crashed immediately against SQLite)
