...
# These models use Postgres-specific types (UUID, JSONB) throughout, so
# SQLite cannot run this schema at all (it raises CompileError on UUID).
# Tests therefore require a real Postgres database. Point TEST_DATABASE_URL
# at a throwaway database - never run these against production data, since
# every table is dropped and recreated per test.
